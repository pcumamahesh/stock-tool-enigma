...
def fetch_data(ticker):
    """
    Fetch intraday stock data for the given ticker.
    By default, 1-minute interval for the past 1 day (or adjust as needed).
    """
    # Get 1-minute data for the past 1 day (you can adjust the period and interval if needed)
    stock_data = yf.download(ticker, period="1d", interval="1m")
    ...
    # We will only use the 'Close' price for this analysis
    stock_data = stock_data[['Close']].dropna()  # Drop NaN values for consistency
    return stock_data

def add_technical_indicators(stock_data):
    """
    Add technical indicators like RSI and MACD to the stock data.
    """
...
def engineer_features(stock_data):
    """
    Create lag-based features to help with prediction (e.g., Close price from previous minutes).
    """
    stock_data['Close_Lag1'] = stock_data['Close'].shift(1)
    stock_data['Close_Lag2'] = stock_data['Close'].shift(2)
...
def split_data(stock_data):
    """
    Split the data into features (X) and target (y), and then further split into train and test sets.
    """
    X = stock_data[['Close_Lag1', 'Close_Lag2', 'RSI', 'MACD']]
    y = stock_data['Close']
...
def train_model(X_train, y_train, X_test, y_test):
    """
    Train a RandomForestRegressor model and return the trained model and its performance metrics.
    """
    model = RandomForestRegressor(n_estimators=600, max_depth=30, random_state=42)
    model.fit(X_train, y_train.values.ravel())
...
def predict_intraday_trend_and_price(model, stock_data):
    """
    Use the trained model to predict the next price and trend direction.
    """
    last_row = stock_data.iloc[-1]
